...
(
    ...
    number        varchar(20) collate utf8mb4_unicode_ci not null comment 'Building number',
    name          varchar(100) collate utf8mb4_unicode_ci default null comment 'Building name',
    ...
    default_order int not null default 0 comment 'Default display order',
    created_at    timestamp not null default CURRENT_TIMESTAMP,
    updated_at    timestamp not null default CURRENT_TIMESTAMP on update CURRENT_TIMESTAMP,
    primary key (id),
    constraint uk_building_v2_number unique (number),
    constraint uk_building_v2_default_order unique (default_order)
) engine = InnoDB
  default charset = utf8mb4
  collate = utf8mb4_unicode_ci;

insert into building_v2 (number, name, default_order)
values ('63동', '학생회관', 1),
       ('75-1동', '전망대', 2),
       ('919동', '관악생활관', 3),
       ('901동', null, 4),
...
alter table restaurant_v2
    ...
    add column default_order int not null default 0 after owner_id;
...
insert into restaurant_v2 (building_id, name, default_order)
select b.id, x.name, x.default_order
from building_v2 b
join (
    select '63동' building_number, '학생회관식당' name, 1 default_order union all
...
alter table restaurant_v2
    ...
    add constraint uk_restaurant_v2_building_order unique (building_id, default_order),
    add constraint uk_restaurant_v2_id_building unique (id, building_id),
...
create table if not exists restaurant_like_v2
(
    user_id       int                                 not null comment 'User ID',
    ...
    constraint restaurant_like_v2_user_fk
        foreign key (user_id) references user (id)
            on delete cascade,
    constraint restaurant_like_v2_restaurant_fk
...
create index restaurant_like_v2_restaurant_id_index
    on restaurant_like_v2 (restaurant_id);
